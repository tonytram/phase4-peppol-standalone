...
public class PeppolSenderController
{
                                       ...
                                       @PathVariable final boolean isInbound)
  {
    ...
    try
    {
      PeppolSBDHDataReader sbdhDataReader = new PeppolSBDHDataReader (PeppolIdentifierFactory.INSTANCE);
      sbdhDataReader.setCheckForCountryC1 (false);
      aData = sbdhDataReader.extractData (new NonBlockingByteArrayInputStream (aPayloadBytes));
    }
    ...
    // If C1 country code is missing, use a placeholder to satisfy downstream validation
    if (!aData.hasCountryC1 ())
      aData.setCountryC1 ("ZZ");
