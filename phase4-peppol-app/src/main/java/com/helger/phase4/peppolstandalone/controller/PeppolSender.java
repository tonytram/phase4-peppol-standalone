...
public final class PeppolSender {
     ...
     *
     * @param aSmlInfo       The SML to be used for receiver lookup
     * @param aAPCAChecker   The Peppol CA checker to be used.
     * @param aPayloadBytes  The main business document to be send
     * @param sSenderID      The Peppol sender Participant ID
     * @param sReceiverID    The Peppol receiver Participant ID
     * @param sDocTypeID     The Peppol document type ID
     * @param sProcessID     The Peppol process ID
     * @param sCountryCodeC1 The Country Code of the sender (C1)
    ...
    public static Phase4PeppolSendingReport sendPeppolMessageCreatingSbdh(@NonNull final ISMLInfo aSmlInfo,
                                                                          ...
                                                                          @Nonempty final boolean isInbound) {
        ...
        try {
            ...
            if (isInbound) {
                ...
                aBuilder.receiverEndpointDetails(certificate, "http://localhost:8080/as4");
     ...
     * @param aSmlInfo       The SML to be used for receiver lookup
     * @param aAPCAChecker   The Peppol CA checker to be used.
     * @param aPDFBytes      The main PDF document to be send
     * @param sSenderID      The Peppol sender Participant ID
     * @param sReceiverID    The Peppol receiver Participant ID
     * @param sCountryCodeC1 The Country Code of the sender (C1)
     * @return The created sending report and never <code>null</code>.
     ...
     *
     * @param aData          The Peppol SBDH data to be send
     * @param aSmlInfo       The SML to be used for receiver lookup
     * @param aAPCAChecker   The Peppol CA checker to be used.
     * @param aSendingReport The sending report to be filled.
     */
    static void sendPeppolMessagePredefinedSbdh(@NonNull final PeppolSBDHData aData,
        ...
        try {
            ...
            // Build SBDH XML, then strip the COUNTRY_C1 Scope element so the
            // receiver sees no C1 country code (for testing missing-C1 scenarios)
            final org.unece.cefact.namespaces.sbdh.StandardBusinessDocument aJaxbSbdh =
                    new com.helger.peppol.sbdh.PeppolSBDHDataWriter ().createStandardBusinessDocument (aData);
            aJaxbSbdh.getStandardBusinessDocumentHeader ()
                      .getBusinessScope ()
                      .getScope ()
                      .removeIf (s -> "COUNTRY_C1".equals (s.getType ()));
            final byte [] aSBDHBytes = new com.helger.sbdh.SBDMarshaller ().getAsBytes (aJaxbSbdh);
            if (aSBDHBytes == null)
                throw new IllegalArgumentException ("Failed to serialize SBDH data");

            final PeppolUserMessageSBDHBuilder aBuilder = Phase4PeppolSender.sbdhBuilder()
                    .httpClientFactory(aHCS)
                    .senderParticipantID(aData.getSenderAsIdentifier())
                    .receiverParticipantID(aData.getReceiverAsIdentifier())
                    .documentTypeID(aData.getDocumentTypeAsIdentifier())
                    .processID(aData.getProcessAsIdentifier())
                    .countryC1(aData.getCountryC1 ())
                    .payload(aSBDHBytes)
                    .senderPartyID(sMyPeppolSeatID)
                    .peppolAP_CAChecker(aAPCAChecker)
                    .smpClient(aSMPClient)
